...
| 8 | done (integrations Phase 0) | Platform OAuth clients — env-driven Tyrbo-owned OAuth client per piece, replacing the deleted EE oauth-apps table so users click **Connect** instead of pasting a client ID. Config: `AP_TYRBO_OAUTH_<PROVIDER>_CLIENT_ID/_CLIENT_SECRET` (provider→pieces map; GOOGLE covers gmail+sheets with launch scope trims per Farebear/tyrbo `docs/GOOGLE-OAUTH.md`) + `AP_TYRBO_OAUTH_CLIENTS` JSON per-piece override incl. optional authorize-time scope clamp (`app/tyrbo/tyrbo-oauth-clients.ts`, clamp applied in `oauth2-util.ts`). CE `GET /v1/oauth-apps` (client ids only) + real `PLATFORM_OAUTH2` claim/refresh installed via upstream's `setPlatformOAuthService` seam, delegating to the community credentials service with the secret injected server-side; stored values never embed the secret (`app/tyrbo/tyrbo-oauth-apps.ts`, registered in `app.ts`). Web: `oauth-apps-hooks.ts` drops the community short-circuit and keeps cloud OAuth (secrets.activepieces.com) off; unconfigured OAuth pieces keep the BYO client-id form. `ce-defaults.ts` `getThirdPartyRedirectUrl` now returns `{AP_FRONTEND_URL}/redirect` (was `undefined` → dialogs showed `no_redirect_url_found`). Public `GET /v1/oauth-apps/pieces` (piece names only, no secrets) lets the product web derive its connector-gallery "coming soon" badge from this single source of truth instead of a hand-maintained env list |
